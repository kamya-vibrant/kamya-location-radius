...
let MSL = {
	...
	main: {
		...
		cleanForm: async (form) => {
            ...
            if (formErrors == 0) {
                //$(".alert_wrapper").html("<div class='alert alert-success'>Recipe has been updated.</div>");
                window.scrollTo({
                    top: 0,
                    left: 0,
	...
	product: {
				...
				if (response['code'] == 200 && response['data'] != '') {
					...
					$.map(response['data'], (item, index) => {
						...
						if (item.image.length > 0) {
                            $('.product_image').attr({
                                "src": item.image,
                                "style": "visibility: visible;margin-left: -100%;"
                            });
		...
		saveProduct: async (e) => {
			console.dir("saveProduct begin:: ");

			let form = $("#"+$(e).attr('id'));

			let checkFormValidity = MSL.main.checkFormValidity(form);
			MSL_GLOBAL.main.globalModal('warning', 'submission', 'Update Recipe', 'Updating recipe... Please wait...');
			...
			if (checkFormValidity === true) {
				console.dir("form is valid...")
				//$('.btn_submit').text("Saving...").attr("disabled", "disabled");
				console.dir("prodID: " + prodID);
				parameter = {
					model: 'product',
					action: 'update',
					update: {},
				...
				if ($('.product_image').attr('src') && form.find('input#is_changed_product').val() == 1) {
					console.dir("new image set...")
					...
					console.dir("blank image set...")
	                productImage = '';
	            }

				parameter.update['name'] = form.find('input[name=product_name]').val();
	            parameter.update['status'] = form.find('input[name=product_status]').val();
				...
				try {
					...
					if (response['code'] == 200 && response['id'] != 0) {
						...
						if (productImage.length > 0) {
							sendSL = await MSL_GLOBAL.main.uploadImageSequentially('product', 'product', prodID, base64Parts, 1, 'image', 'main-product-form');
	                    } else {
	                        if (formErrors == 0) {
	                            setTimeout(() => {
					...
					MSL_GLOBAL.main.globalModal('danger', 'failed', 'Update Recipe', 'We encountered an error. Please try again or contact support with error: ' + error);
					console.log("Error: ", error);
					MSL.main.cleanForm('product');
				}
			} 
			MSL_GLOBAL.main.globalModal('success', 'succesful', 'Update Recipe', 'Recipe updated successfully.');
			console.dir("saveProduct end:: ");
