...
let Inventory = {
	...
	main: {
						...
						ingredients += `<tr class="ingredient-row-${item.id}"> \
				                            ...
				                            <td> \
				                                <div class="button-group xy-center" style="gap: 10px;"> \
				                                    <a href="/edit-recipe?id=${item.id}" class="btn default icon-right" style="">Edit <i class="bi bi-pencil-fill"></i></a> \
		...
		removeIngredient: async (e) => {
			...
			if (confirm('Are you sure you want to remove this recipe? Thi action can\'t be undone.')) {
